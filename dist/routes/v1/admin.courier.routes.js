...
import { steadfastCreateConsignment, steadfastBulkCreate, steadfastTrackByConsignment, steadfastTrackByInvoice, steadfastTrackByTracking, steadfastBalance, steadfastCreateReturnRequest, steadfastGetReturnRequest, steadfastGetReturnRequests, steadfastGetPayments, steadfastGetPayment, steadfastGetPoliceStations, } from "../../services/steadfast.service.js";
import { pathaoCreateOrder, pathaoCreateBulkOrders, pathaoTrack, pathaoCancelOrder, pathaoGetCities, pathaoGetZones, pathaoGetAreas, pathaoGetStores, pathaoCreateStore, pathaoGetPriceplan, } from "../../services/pathao.service.js";
const router = Router();
    ...
    try {
        ...
        if (!result || result.status !== 200) {
            return res.status(400).json({
                ok: false,
                message: result?.message ?? "Steadfast rejected the order",
                data: result,
            });
        }
...
router.post("/courier/steadfast/bulk-send", async (req, res, next) => {
    try {
        ...
        if (!raw || (raw.status !== undefined && raw.status !== 200)) {
            return res.status(400).json({
                ok: false,
                message: raw?.message ?? "Steadfast rejected the bulk order",
                data: raw,
            });
        }
        const resultArray = Array.isArray(raw)
            ? raw
...
router.get("/courier/shipments", async (req, res, next) => {
    try {
        ...
        const filter = { "courier.consignmentId": { $exists: true } };
        if (provider)
