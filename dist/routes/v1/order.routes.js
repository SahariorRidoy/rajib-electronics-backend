...
router.post("/orders", async (req, res) => {
    ...
    try {
        ...
        for (const line of normalized) {
            ...
            const updated = await Product.findOneAndUpdate({ _id: line._id, stock: { $gte: line.quantity } }, {
                $inc: {
                    stock: -line.quantity,
        ...
        try {
            ...
            await NotificationService.createOrderNotification(String(createdOrder._id), orderData.customer.name, orderData.totals.grandTotal);
        }
...
router.get("/orders", async (req, res) => {
    try {
        ...
        const phones = [...new Set(items.map((o) => o.customer?.phone).filter(Boolean))];
        ...
        const formatted = items.map((o) => {
            ...
            const allForPhone = phone ? (ordersByPhone[phone] ?? []) : [];
            ...
            const sameDay = allForPhone.some((x) => String(x._id) !== String(o._id) &&
            ...
            const returning = allForPhone.some((x) => String(x._id) !== String(o._id) &&
...
router.get("/orders/by-phone/:phone", async (req, res) => {
    try {
        ...
        const formatted = orders.map((o) => ({
            ...
            itemCount: Array.isArray(o.lines) ? o.lines.reduce((s, l) => s + (l.qty ?? 1), 0) : 0,
            lines: Array.isArray(o.lines) ? o.lines.map((l) => ({ title: l.title, qty: l.qty, price: l.price })) : [],
...
router.patch("/orders/:id/lines", async (req, res) => {
    try {
        ...
        // Recalculate totals
        const subTotal = lines.reduce((sum, l) => sum + (Number(l.price) * Math.max(1, Number(l.qty))), 0);
...
router.post("/orders/:id/notes", async (req, res) => {
    try {
        ...
        const { text } = req.body;
        if (!text?.trim())
            return res.status(400).json({ ok: false, message: "Note text required" });
        const updated = await Order.findByIdAndUpdate(id, { $push: { adminNotes: { text: text.trim(), createdAt: new Date() } } }, { new: true }).lean();
        if (!updated)
            return res.status(404).json({ ok: false, message: "Order not found" });
        const formatted = {
            ...updated,
...
router.delete("/orders/:id/notes/:noteIndex", async (req, res) => {
    try {
        ...
        if (idx < 0 || idx >= notes.length)
            return res.status(400).json({ ok: false, message: "Invalid note index" });
        notes.splice(idx, 1);
        const updated = await Order.findByIdAndUpdate(id, { adminNotes: notes }, { new: true }).lean();
        const formatted = {
            ...updated,
            _id: String(updated._id),
            lines: updated.lines.map((l) => ({ ...l, productId: String(l.productId) })),
