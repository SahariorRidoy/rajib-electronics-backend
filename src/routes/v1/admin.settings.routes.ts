...
router.patch("/settings", requireAdmin, async (req, res, next) => {
  try {
    ...
    if (!settings) settings = await SiteSettings.create({ siteName });
    else { settings.siteName = siteName; await settings.save(); }
    res.json({ ok: true, data: settings });
  } catch (err) {
    next(err);
  }
...
router.patch("/settings/contact", requireAdmin, async (req, res, next) => {
  try {
    ...
    let settings = await SiteSettings.findOne();
    if (!settings) settings = await SiteSettings.create({});
    if (body.phones !== undefined) settings.contactInfo.phones = body.phones;
    if (body.emails !== undefined) settings.contactInfo.emails = body.emails;
    await settings.save();
...
router.patch("/settings/social/:id", requireAdmin, async (req, res, next) => {
  try {
    ...
    const settings = await SiteSettings.findOne();
    if (!settings) return res.status(404).json({ ok: false, code: "NOT_FOUND" });
    const link = settings.socialLinks.find((s: any) => s._id.toString() === req.params.id);
    if (!link) return res.status(404).json({ ok: false, code: "SOCIAL_NOT_FOUND" });
    if (body.platform !== undefined) link.platform = body.platform;
    if (body.value !== undefined) link.value = body.value;
...
router.delete("/settings/social/:id", requireAdmin, async (req, res, next) => {
  try {
    ...
    const settings = await SiteSettings.findOne();
    if (!settings) return res.status(404).json({ ok: false, code: "NOT_FOUND" });
    const idx = settings.socialLinks.findIndex((s: any) => s._id.toString() === req.params.id);
    if (idx === -1) return res.status(404).json({ ok: false, code: "SOCIAL_NOT_FOUND" });
...
router.patch("/settings/logos/:logoId/activate", requireAdmin, async (req, res, next) => {
  try {
    ...
    const logo = settings.logos.find((l: any) => l._id.toString() === req.params.logoId);
    if (!logo) return res.status(404).json({ ok: false, code: "LOGO_NOT_FOUND" });
...
router.delete("/settings/logos/:logoId", requireAdmin, async (req, res, next) => {
  try {
    ...
    const logoIndex = settings.logos.findIndex((l: any) => l._id.toString() === req.params.logoId);
    if (logoIndex === -1) return res.status(404).json({ ok: false, code: "LOGO_NOT_FOUND" });
