...
async function createOrderFromPayload(payload: any, paymentInfo: any, deliveryChargePaid: boolean) {
...
router.post("/payment/initiate", async (req, res) => {
  try {
    ...
    const deliveryConfig = await DeliverySettings.findOne().lean() as any;
...
async function handleCallback(req: any, res: any) {
  try {
    ...
    if (isSuccess) {
      ...
      const deliveryChargePaid = !!(pending as any).deliveryChargePaid;
      ...
      return res.redirect(
        `${frontendUrl}/payment/success?orderId=${order._id}&invoice=${invoice_number}&trxId=${trx_id || ""}&amount=${payment_amount || ""}`
      );
    } else {
      ...
      return res.redirect(
        `${frontendUrl}/payment/failed?reason=payment_failed`
      );
    }
  } catch (err) {
